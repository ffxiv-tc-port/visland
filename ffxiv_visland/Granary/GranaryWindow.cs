...
unsafe class GranaryWindow : UIAttachedWindow {
    ...
    // 🔑 主導項是「收納袋現有 − 工坊兩週需求 最低」:權重 1 << (TopScarce-1-名次),所以較低名次全部加起來也贏不過任何一個更高的名次(512 > 256+128+...+1 = 511)—— 覆蓋到最缺那一項的遠征地必定勝出。這正是使用者要的語意:最缺鐵礦就該派往「山」,而不是被一堆中等材料的廣度蓋過去。
    // 🔑 需求 0 的材料鍵值就等於它的庫存,照樣參與排名 —— 所以「鐵礦沒被排程吃到就被無視」那個舊 bug 不會因為扣需求而回來(它的成因是拿缺口當篩選器,不是扣需求本身)。
    // 工坊缺口保留為次要項:主導項雖然已經把需求吃進去了,但缺口多扣了在途、且不受前 N 名限制,所以在主導項完全平手(例如兩個遠征地都沒覆蓋到前 N 名)時仍有鑑別力。
