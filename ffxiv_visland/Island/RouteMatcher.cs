...
// 把「已儲存的採集路線」對上「無人島材料」。
// 🔴 只能用座標圓比對 —— 另外三條看起來更直接的路都會**靜默**失敗:① 路線名稱是使用者自己取的,實測整批是簡體中文用語,跟任何遊戲資料表都對不上;② Waypoint.InteractWithName 存的是遊戲物件名(英/法文),不是材料名;③ Waypoint.InteractWithOID 全島只有兩個值(2012985 佔 257 筆、2013159 佔 20 筆),所有採集點共用同一個 OID,分不出採的是什麼;④ 394 個路徑點裡有 298 個 ZoneID = 0,拿區域 id 過濾會濾掉四分之三。
// 座標的依據:MJIGatheringItem 的 X / Y 兩欄實際上是世界座標的 X / Z(表上叫 Y),半徑就是同列的 Radius。
// ⚠️ 已知限制:採集圓大量重疊(例如石材/石灰岩/大理石都在 (-22, -46) 半徑 80,石英的半徑更是 200),所以一條路線會命中它實際上不採的材料。這裡不猜、不過濾,照實把覆蓋集合顯示出來。
