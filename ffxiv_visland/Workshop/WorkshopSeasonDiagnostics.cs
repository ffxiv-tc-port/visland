...
// 季號對位診斷。
// 問題:WorkshopSeasonDB.SeasonForWeek 純粹用日期算術推季號,從來沒有跟遊戲實際的資料對照過。如果相位差了,現在載入的那 5 天就是**錯的季**——照樣會有貝殼幣、只是比較少,所以壞掉的方式是靜默的。
// 為什麼不離線定錨:試過了,不行。封存排程對受歡迎度幾乎不敏感。也就是說「用受歡迎度反推季號」這條路本身不成立,不是查詢寫壞了。
// 所以改成:把「算出來的季號」與「遊戲實際的受歡迎度列號」同時擺出來,讓實機一次觀測就能定錨。使用者跑 LogLevel 1 → 一律寫 Information。
